...
class Model
{
    ...
    public function load($id)
    {
        ...
        if ($this->data) {
            ...
            // Load Additional Attributes (EAV simulation for compatibility)
            // In a full refactor, this should be cleaner, but we must maintain existing logic.
    ...
    public function loadByUrlKey($urlKey)
    {
        ...
        if ($this->data) {
            ...
            // Normalize Stock
            // Force reset if invalid or missing
            if (!isset($this->data['stock_count']) || $this->data['stock_count'] === '' || $this->data['stock_count'] === null) {
                $this->data['stock_count'] = (!empty($this->data['in_stock']) && $this->data['in_stock'] == 1) ? 1000 : 0;
            }
        }
    ...
    {
        // Fetch simulated EAV attributes
        $query = new Query();
