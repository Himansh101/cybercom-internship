...
const Renderer = {
    ...
    renderTaskActions(task) {
        ...
        // Show complete button only if user can complete this specific task
        // Check both permission AND task status
        if (task.assignedTo && task.status === 'IN_PROGRESS') {
            const canComplete = RulesEngine.canCompleteTask(task);
            if (canComplete.allowed) {
                actions += `
                ...
                // Show why they can't complete (for debugging/clarity)
                if (Auth.isManager()) {
                    actions += `
                        <p style="margin-top: 10px; color: #6c757d; font-size: 13px; font-style: italic;">
                            ℹ️ Only assigned interns or admins can mark tasks as complete.
                        </p>
                    `;
                }
            }
